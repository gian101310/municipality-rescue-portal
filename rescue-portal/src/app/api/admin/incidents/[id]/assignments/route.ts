...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  // If reassigning, release the previous team back to available
  if (incident.assigned_unit_id && incident.assigned_unit_id !== team.id) {
    await admin.from('rescue_units').update({ status: 'available', updated_at: now }).eq('id', incident.assigned_unit_id)
    // Cancel the old assignment record
    await admin.from('incident_assignments').update({ status: 'cancelled' }).eq('incident_id', id).eq('rescue_unit_id', incident.assigned_unit_id).eq('status', 'assigned')
  }
  const { data, error } = await admin.from('incidents').update({ assigned_unit_id: team.id, assigned_unit_name: team.name, status: 'assigned', updated_at: now }).eq('id', id).select('*').single(); if (error) return NextResponse.json({ message: error.message }, { status: 500 })
