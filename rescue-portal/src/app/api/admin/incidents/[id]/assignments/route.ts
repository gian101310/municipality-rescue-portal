...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { data, error } = await admin.from('incidents').update({ assigned_unit_id: team.id, status: 'assigned', updated_at: now }).eq('id', id).select('*').single(); if (error) return NextResponse.json({ message: error.message }, { status: 500 })
