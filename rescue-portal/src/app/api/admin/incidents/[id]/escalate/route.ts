...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!profile?.is_active || !['super_admin','admin','dispatcher','verifier'].includes(profile.role)) return NextResponse.json({ message: 'Incident update access required.' }, { status: 403 })
