...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const { error } = await admin.from('rescue_unit_members').upsert({ unit_id: id, user_id: member.id, user_name: member.full_name, role: body.position, is_active: true, joined_at: new Date().toISOString(), left_at: null }, { onConflict: 'unit_id,user_id' }).select('*').single()
